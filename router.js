...
class AgentRouter {
  ...
  // ── ROUTE TASK ────────────────────────────────────────────────────────────
  // Main entry point — takes a task and returns the right agent + result
  async route(task, options = {}, context = {}) {
    const agentName = await this._selectAgent(task, options, context);

    // Workflow agent = original tejas run logic (shell commands)
    // Return the name so run.js can handle it natively
    if (agentName === 'workflow' || agentName === 'system') {
      return { agent: agentName, useNativeExecutor: true };
    }

    const agent = this.agents[agentName];
    if (!agent) {
      return { agent: 'workflow', useNativeExecutor: true };
    }

    const result = await agent.run(task, context);
    return { agent: agentName, useNativeExecutor: false, result };
  }
  ...
  getAgentList() {
    return [
      ...
      },
      {
        name:        'voice',
        status:      'planned',
        description: 'Natural voice interface — Jarvis mode',
