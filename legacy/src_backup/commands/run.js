...
module.exports = async function run(task, options) {
  ...
  try {
    ...
    // ── ROUTE ───────────────────────────────────────────────────────────────────
    let routing = { agent: 'workflow' };
    try {
      routing = await router.route(task, options);
    } catch {
      routing = { agent: 'workflow' };
    }

    // ── AGENT PATH (file/code/web) ───────────────────────────────────────────────
    // Workflow and system always use the native executor (planning + shell)
    if (routing.agent !== 'workflow' && routing.agent !== 'system') {
      const agent = router.agents[routing.agent];
      if (agent) {
        spin.stop();
        const start = Date.now();
        const result = await agent.run(task, { user: config.user || {} });
    ...
    // Confirm if needed
    const hasShell = plan.steps.some(function(s) { return s.action === 'shell'; });
    if (plan.requires_confirmation || hasShell) {
      try {
        const answer = await inquirer.prompt([{
          type:    'confirm',
          name:    'confirmed',
          message: 'Execute ' + plan.steps.length + ' steps?',
          default: true
        }]);
        if (!answer.confirmed) {
          display.warn('Task cancelled.');
          return;
        }
      } catch {
    ...
    if (plan.memory_update && plan.memory_update.should_learn &&
      ...
      try {
        const learn = await inquirer.prompt([{
          type:    'confirm',
          name:    'shouldSave',
          message: 'Save "' + plan.memory_update.pattern_name + '" as learned workflow?',
          default: true
        }]);
        if (learn.shouldSave) {
          ...
          display.success('Learned: "' + plan.memory_update.pattern_name + '"');
        }
